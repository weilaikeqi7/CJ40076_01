/**
 * @file app.c
 * @brief CJ40076 V2.0（N32L40x）应用主状态机实现（与 03 相同结构，
 *        差异：无显示屏电源、无加热丝、无 NTC 温度）
 */
#include "app.h"
...
void app_run(void* argument)
{
    ...
    LOGI("sys: CJ40076 V2.0 (N32L40x) boot\r\n");

    board_adc_init();
    store_init();
    attitude_load_offsets();

    display_init();       /* LCD（CS1622）常供电 */
