/**
 * @file app_attitude.h
 * @brief 姿态解算：MCP-406 原始角度 -> 补偿后俯仰/航向（0.01 度整数运算）
 *
 * 换算规则：
 *   俯仰 = 原始俯仰 + PIt      （激光朝上为正、朝下为负，-90°~+90°）
 *   航向 = 原始航向 + HIt + HEr（归一化到 0~359.99°，北为0°，顺时针为正）
 * MCP-406 按"Y 轴朝上 180°"方式安装（安装方式=12），10Hz 连续输出。
 */
